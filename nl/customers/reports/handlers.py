...
def ahead():
    ...
    if form.validate_on_submit():
        ...
        qry = Customer.query\
                      .join(Sequence, Customer.id==Sequence.tag_id)\
...
def info():
    ...
    if form.validate_on_submit():
        ...
        report = []
        for c in records:
            names = Name.query.filter(Name.customer_id==c.id)\
                              .order_by(Name.sequence)\
                              .all()
            addresses = Address.query.filter(Address.customer_id==c.id)\
                                     .order_by(Address.sequence)\
                                     .all()
            telephones = Telephone.query.filter(Telephone.customer_id==c.id)\
            ...
            if form.adjustments.data:
                adjrecs = Adjustment.query\
                                    .filter(Adjustment.customer_id==c.id)\
                                    .order_by(Adjustment.period_id,
                                              Adjustment.created,
                                              Adjustment.updated)\
                                    .all()
                adjustments = []
            ...
            if form.payments.data:
                pmtrecs = Payment.query\
                                 .filter(Payment.customer_id==c.id)\
                                 .order_by(Payment.period_id,
                                           Payment.date,
                                           Payment.created,
                                           Payment.updated)\
                                 .all()
