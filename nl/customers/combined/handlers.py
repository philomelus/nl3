...
def index():
    ...
    form = CombinedForm
    primary = db.session.execute(select(distinct(CombinedBill.customer_id_main))).scalars().all()
    combined = []
    for p in primary:
        info = Customer.query.filter_by(id=p).first()
        others =[]

        secondaries = db.session.execute(select(distinct(CombinedBill.customer_id_secondary))
                                         .filter(CombinedBill.customer_id_main==p)).scalars().all()
