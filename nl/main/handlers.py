...
def index():
    ...
    totalCount = Customer.query.filter(
        or_(Customer.routeList == "Y", Customer.active == "Y")
    ).count()
    activeAndOwe = Customer.query.filter(
        and_(Customer.active == "Y", Customer.balance > 0)
    ).count()
    activeAndAhead = Customer.query.filter(
        and_(
            or_(Customer.active == "Y", Customer.routeList == "Y"),
            Customer.balance <= 0,
        )
    ).count()
    period = Config.get("billing-period")
    title = Period.query.filter_by(id=period).first().title
    lastBills = Bill.query.filter(and_(Bill.export == "Y", Bill.iid == period)).count()
    ...
    routes = [
        r[0]
        for r in db.session.query(Route.id)
        .filter(Route.active == "Y")
        .order_by(Route.title)
        .all()
    ]
    routeTitles = [
        r[0]
        for r in db.session.query(Route.title)
        .filter(Route.active == "Y")
        .order_by(Route.title)
        .all()
    ]
