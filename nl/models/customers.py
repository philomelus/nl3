...
class Customer(db.Model):
    ...
    route_id = db.Column(
        db.ForeignKey("routes.id", ondelete="RESTRICT", onupdate="CASCADE"),
        nullable=False,
        index=True,
    )
    type_id = db.Column(
        db.ForeignKey("customers_types.id", ondelete="RESTRICT", onupdate="CASCADE"),
        nullable=False,
        index=True,
    )
    ...
    billPeriod = db.Column(
        db.ForeignKey("periods.id", ondelete="RESTRICT", onupdate="CASCADE"),
        nullable=True,
        index=True,
    )
    ...
    lastPayment = db.Column(
        db.ForeignKey("customers_payments.id", ondelete="RESTRICT", onupdate="CASCADE"),
        nullable=True,
        index=True,
    )
    ...
    period = db.relationship(
        "Period", primaryjoin="Customer.billPeriod == periods.c.id"
    )
    route = db.relationship(
        "Route", primaryjoin="Customer.route_id == routes.c.id", backref="customers"
    )
    type = db.relationship(
        "Type", primaryjoin="Customer.type_id == customers_types.c.id"
    )
    ...
    def rate(self):
        ...
        r = (
            Rate.query.filter(Rate.type_id == self.type_id)
            .filter(Rate.period_id_begin <= period)
            .filter(or_(Rate.period_id_end >= period, Rate.period_id_end == None))
            .first()
        )
...
class Address(db.Model):
    ...
    customer_id = db.Column(
        db.ForeignKey("customers.id", ondelete="RESTRICT", onupdate="CASCADE"),
        primary_key=True,
    )
    ...
    customer = db.relationship(
        "Customer",
        primaryjoin="Address.customer_id == customers.c.id",
        backref="addresses",
        lazy="noload",
    )
...
class Adjustment(db.Model):
    ...
    customer = db.relationship(
        "Customer",
        primaryjoin="Adjustment.customer_id == customers.c.id",
        backref="adjustments",
        lazy="noload",
    )
    period = db.relationship(
        "Period", primaryjoin="Adjustment.period_id == periods.c.id"
    )
...
class Bill(db.Model):
    ...
    period = db.relationship(
        "Period", primaryjoin="Bill.iid == Period.id", lazy="noload"
    )
...
class BillLog(db.Model):
    ...
    customer = db.relationship(
        "Customer",
        primaryjoin="BillLog.customer_id == Customer.id",
        backref="bill_logs",
        lazy="noload",
    )
    period = db.relationship(
        "Period", primaryjoin="BillLog.period_id == Period.id", lazy="noload"
    )
...
class CombinedBill(db.Model):
    ...
    customer_id_main = db.Column(
        db.ForeignKey("customers.id", ondelete="RESTRICT", onupdate="CASCADE"),
        primary_key=True,
    )
    customer_id_secondary = db.Column(
        db.ForeignKey("customers.id", ondelete="RESTRICT", onupdate="CASCADE"),
        primary_key=True,
    )
    created = db.Column(db.DateTime, nullable=False)
...
class Complaint(db.Model):
    ...
    period_id = db.Column(
        db.ForeignKey("periods.id", ondelete="RESTRICT", onupdate="CASCADE"), index=True
    )
    created = db.Column(db.DateTime, nullable=False)
    updated = db.Column(db.DateTime, nullable=False)
    type = db.Column(
        db.Enum("MISSED", "WET", "DAMAGED", "LATE", "OTHER"), nullable=False
    )
    when = db.Column(db.Date, nullable=False)
    why = db.Column(db.String(255), nullable=False)
    result = db.Column(
        db.Enum(
            "NONE", "CREDITDAILY", "CREDITSUNDAY", "REDELIVERED", "CREDIT", "CHARGE"
        ),
        nullable=False,
    )
    ...
    customer = db.relationship(
        "Customer",
        primaryjoin="Complaint.customer_id == Customer.id",
        backref="complaints",
        lazy="noload",
    )
...
class Name(db.Model):
    ...
    customer = db.relationship(
        "Customer", primaryjoin="Name.customer_id == Customer.id", backref="names"
    )
...
class Payment(db.Model):
    ...
    customer = db.relationship(
        "Customer",
        primaryjoin="Payment.customer_id == Customer.id",
        backref="payments",
        lazy="noload",
    )
...
class Rate(db.Model):
    ...
    period_id_begin = db.Column(
        db.ForeignKey("periods.id", onupdate="CASCADE"), nullable=False, index=True
    )
    period_id_end = db.Column(
        db.ForeignKey("periods.id", onupdate="CASCADE"), nullable=True, index=True
    )
    created = db.Column(db.DateTime, nullable=False)
    updated = db.Column(db.DateTime, nullable=False)
    ...
    type = db.relationship(
        "Type", primaryjoin="Rate.type_id == Type.id", backref="rates", lazy="noload"
    )
...
class ServiceChange(db.Model):
    ...
    customer = db.relationship(
        "Customer",
        primaryjoin="ServiceChange.customer_id == Customer.id",
...
class ServiceType(db.Model):
    ...
    type_id_from = db.Column(
        db.ForeignKey("customers_types.id", ondelete="RESTRICT", onupdate="CASCADE"),
        nullable=False,
        index=True,
    )
    type_id_to = db.Column(
        db.ForeignKey("customers_types.id", ondelete="RESTRICT", onupdate="CASCADE"),
        nullable=False,
        index=True,
    )
    ...
    customer = db.relationship(
        "Customer",
        primaryjoin="ServiceType.customer_id == Customer.id",
...
class Telephone(db.Model):
    ...
    customer = db.relationship(
        "Customer",
        primaryjoin="Telephone.customer_id == Customer.id",
        backref="telephones",
        lazy="noload",
    )
