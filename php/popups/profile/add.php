	...
	if (isset($_REQUEST['action']))
	{
		if ($_REQUEST['action'] == 'Add')
		{
			...
			do
			{
				...
				$result = db_insert('users_configuration', $fields);
				if (!$result)
				{
				...
				audit('Added profile setting. ' . audit_add($fields));
			} while (false);
		}
	}
	else
...
//-----------------------------------------------------------------------------
	...
	echo gen_htmlHeader('Add Profile', $styles, $script);
	...
	if (isset($_POST['key1']) && $_POST['key1'] == 'All')
		$selecteds['key1'] = ' selected="selected"';
	if (isset($_POST['key2']) && $_POST['key2'] == 'All')
		$selecteds['key2'] = ' selected="selected"';
	if (isset($_POST['key3']) && $_POST['key3'] == 'All')
		$selecteds['key3'] = ' selected="selected"';
?>
			...
			<tr>
				...
				<td>
					<select name="key1" onchange="JavaScript:update_keys(); this.form.submit();">
						<option<?php echo $selecteds['key1'] ?>>All</option>
<?php
	$temp = &$data->keys1(false);
	foreach($temp as $key)
	{
		if (isset($_POST['key1']) && $_POST['key1'] == $key)
...
?>
					...
					<select name="key2" onchange="JavaScript:update_keys(); this.form.submit();"<?php echo $keys[2] ?>>
						<option<?php echo $selecteds['key2'] ?>>All</option>
<?php
	if (!isset($_POST['key1']) || $_POST['key1'] == 'All')
	{
		foreach($data->keys2(false) as $key)
		{
			if (isset($_POST['key2']) && $_POST['key2'] == $key)
				$selected = ' selected="selected"';
			else
				$selected = '';
			echo '<option ' . $selected . '>' . $key . '</option>';
	...
	{
		$temp = $data->keys2FromKeys1(false, $_POST['key1']);
		foreach($temp as $key)
		{
			if (isset($_POST['key2']) && $_POST['key2'] == $key)
				$selected = ' selected="selected"';
...
?>
					...
					<select name="key3" onchange="JavaScript:update_keys(); this.form.submit();"<?php echo $keys[3] ?>>
						<option<?php echo $selecteds['key3'] ?>>All</option>
<?php
	if (!isset($_POST['key2']) || $_POST['key2'] == 'All')
	{
		foreach($data->keys3(false) as $key)
		{
			if (isset($_POST['key3']) && $_POST['key3'] == $key)
				$selected = ' selected="selected"';
			else
				$selected = '';
			echo '<option ' . $selected . '>' . $key . '</option>';
	...
	{
		$temp = $data->keys3FromKeys2(false, $_POST['key2']);
		foreach($temp as $key)
		{
			if (isset($_POST['key3']) && $_POST['key3'] == $key)
				$selected = ' selected="selected"';
