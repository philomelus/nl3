...
//=============================================================================
	...
	$CONFIGURATION = get_userConfig($_POST['key']);
		...
		if ($_REQUEST['action'] == 'Save Changes')
		{
			...
			do
			{
				...
				if (count($fields2) > 0)
				{
					...
					$result = db_update('users_configuration', $fields, $fields2);
					if (!$result)
					{
					...
					audit('Updated profile ' . $fields['key'] . '. '
...
//-----------------------------------------------------------------------------
	...
	echo gen_htmlHeader('Edit Profile Configuration', $styles, $script);
