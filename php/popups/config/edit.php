...
//=============================================================================
	...
	$CONFIGURATION = get_globalConfig($_POST['key']);
	...
	if (isset($_REQUEST['action']))
	{
		if ($_REQUEST['action'] == 'Save Changes')
		{
			...
			do
			{
				...
				{
				case CFG_BOOLEAN:
					$temp = $_POST['valBOOLEAN'];
					if ($temp != $CONFIGURATION)
						$fields2['value'] = '\'' . $temp . '\'';
					break;

				case CFG_COLOR:
					$temp = hexdec(substr($_POST['color'], 1));
					if (intval($temp) != intval($CONFIGURATION))
						$fields2['value'] = $temp;
					break;

				case CFG_ENUM:
					$temp = intval($_POST['valENUM']);
					if ($temp != intval($CONFIGURATION))
						$fields2['value'] = $temp;
					break;

				case CFG_FLOAT:
					$temp = floatval($_POST['valFLOAT']);
					if ($temp != floatval($CONFIGURATION))
						$fields2['value'] = $temp;
					break;

				case CFG_INTEGER:
					$temp = intval($_POST['valINTEGER']);
					if ($temp != intval($CONFIGURATION))
						$fields2['value'] = $temp;
					break;
				...
				case CFG_PERIOD:
					$temp = intval($_POST['valIID']);
					if ($temp != intval($CONFIGURATION))
						$fields2['value'] = $temp;
					break;

				case CFG_ROUTE:
					$temp = intval($_POST['valRID']);
					if ($temp != intval($CONFIGURATION))
						$fields2['value'] = $temp;
					break;

				case CFG_STRING:
					$temp = stripslashes($_POST['valSTRING']);
					if ($temp != $CONFIGURATION)
						$fields2['value'] = '\'' . db_escape($temp) . '\'';
					break;
				...
				if (count($fields2) > 0)
				{
					...
					$result = db_update('configuration', $fields, $fields2);
					if (!$result)
					{
						$message = '<span>Failed to update setting!</span>';
						break;
					}
					$message = '<span>Changes saved successfully.</span>';
					audit('Updated configuration ' . $fields['key'] . '. '
							. 'value was \'' . $CONFIGURATION . '\', now is ' . $fields2['value'] . '.');
				}
				else
...
//-----------------------------------------------------------------------------
	...
	echo gen_htmlHeader('Edit Configuration', $styles, $script);
...
<?php
	...
	case CFG_MONEY:
		if (isset($_REQUEST['valFLOAT']) && !empty($_REQUEST['valFLOAT']))
			$val = floatval($_REQUEST['valFLOAT']);
		else
			$val = $CONFIGURATION;
		echo '<td>'
				. '$<input type="text" name="valFLOAT" value="' . $val . '" size="10" maxlength="10" />';
	...
	case CFG_TELEPHONE:
		if (isset($_POST['telAreaCode']))
		{
			...
			preg_match('/^\(([[:digit:]]{3})\) ([[:digit:]]{3})-([[:digit:]]{4})( [Ee][Xx][Tt] ([[:alnum:]]{0,10}))?$/', $CONFIGURATION, $temp);
		echo '<td>'
				. '(&nbsp;'
