		...
		if ($changesCount > 0)
		{
			while ($change = $changes->fetch_object())
			{
				...
				$changesHtml .= '<tr>'
						...
						. '<td>'
							. ServiceEditLink($change->id, '../../')
								. '<img src="../../img/edit.png" alt="Edit' . $alt . '" title="Edit' . $alt . '" />'
							. '</a>'
						. '</td>';

				$href = $_SERVER['PHP_SELF'] . '?cid=' . $_REQUEST['cid'] . '&menu=' . IDM_STOPSSTARTS . '&hid=' . $change->id . '&action=dc';
				$changesHtml .= '<td>'
				...
				{
				case SERVICE_STOP:
					$changesHtml .= '<td><span>STOP</span></td>';
					break;

				case SERVICE_START:
					$changesHtml .= '<td><span>START</span></td>';
					break;
...
<?php echo gen_dbFields(0, 12, '../../'); ?>
				<input type="submit" name="action" value="Add New" onclick="<?php echo ServiceAddUrl($customer->id, '../../') ?>; return false;" />
			</div>
			<table>
				<thead>
					<tr>
...
';
	...
	function submit()
	{
		...
		do
		{
			...
			$query = "DELETE FROM `customers_service` WHERE `id` = " . $HID . " LIMIT 1";
		...
		if ($err >= ERR_SUCCESS)
		{
			...
			// Note the success
			$message = '<span>Stop/Start ' . sprintf('H%08d', $HID)
					. ' deleted successfully!</span>';

			global $customer;
			audit('Deleted stop/start (id = ' . sprintf('%08d', $HID) . ') from customer '
