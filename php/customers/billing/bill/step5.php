...
*/
	...
	function subsubdisplay()
    {
        ...
        $smarty->assign('action', $_SERVER['PHP_SELF'] . '?menu=4&amp;submenu=1&amp;m3=5');

        $smarty->assign('menu', $_REQUEST['menu']);
        $smarty->assign('submenu', $_REQUEST['submenu']);
		$smarty->assign('m3', $_REQUEST['m3']);

        $smarty->display('customers/billing/bill/step5.tpl');
	}

	function subsubsubmit()
	{
		global $Period, $message, $err;

		if ($_POST['action'] == 'Update Combined Accounts')
		{
			$BILL = new Biller();
			$BILL->Combine($Period[P_PERIOD]);
			unset($BILL);
			$err = ERR_SUCCESS;
			set_globalConfig('billing-status', BILL_COMPLETE);
			audit('Updated combined bills.');
			$message = '<span>Bills have been combined.</span>';
		}
	}
